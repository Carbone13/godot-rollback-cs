...
func _enter_tree():
	...
	_add_setting("debug/multirun/other_window_args", TYPE_STRING, "join1")
	_add_setting("debug/multirun/third_window_args", TYPE_STRING, "join2")
...
func _multirun_pressed():
	...
	for i in range(window_count-1):
		if (i == 2):
			commands = ["--position", str(0) + "," + str((i) * 540)]
		else:
			commands = ["--position", str(((i+1) * window_dist)) + "," + str((i) * 540)]
